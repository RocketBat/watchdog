...
my $refresh_timer = 200; #----speed of logging
...
my $watchdog_log = '/usr/adm/watchdog/logs/bypass.log'; #---CHECK FULL PATH
...
sub bypass_state {
	if (`bpctl_util all get_bypass | grep on | grep -v grep` eq "") {   #<--- CHECK THIS
		$bypass=0;
		print "bypass is off\n";
	}
	else {
